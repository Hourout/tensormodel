    ...
    def _axis_transform_up(self):
        ...
        for i in self._result_up[0]:
            ...
            if sum([1 for char in self._char_name if i[1][0].startswith(char)]):
                ...
                axis_dict['user_address'].append(([x+w*3.5, y+h*7, x+w*21.5, y+h*11.5], 0.4))
                ...
                axis_dict['user_address'].append(([x+w*3.5, y+h*4.5, x+w*21.5, y+h*9.5], 0.6))
                ...
                axis_dict['user_address'].append(([x-w*5.5, y+h*4.5, x+w*12, y+h*9.5], 0.6))
                ...
                axis_dict['user_address'].append(([x+w*3.5, y+h*2.5, x+w*21.5, y+h*7], 0.8))
                ...
                x = i[0][0][0]
                y = i[0][0][1]
                axis_dict['user_name'].append(([x+w*3.5, y-h*8.5, x+w*10, y-h*6], 0.4))
                ...
                axis_true['user_address'] = [x+w*3.5, y-h*0.5, x+w*21.5, y+h*4]
        ...
        for i in self._result_up[0]:
            if len(i[1][0]) in [9, 10, 11] and i[1][0].find('年')==4 and '月' in i[1][0] and i[1][0].endswith('日'):
                ...
                axis_dict['user_address'].append(([x-w*0.1, y+h*2, x+w*1.3, y+h*6], 0.8))
                ...
                axis_dict['user_name'].append(([x-w*0.1, y-h*5.5, x+w*0.5, y-h*3], 0.8))
        ...
        for i in self._result_up[0]:
            ...
            if len(i[1][0])==18 or '号码' in i[1][0] or '公民' in i[1][0]:
                ...
                for char in self._char_nation:
                    if char in i[1][0]:
                        if (i[1][0][i[1][0].find(char)+2:]).strip()!='':
            ...
            if sum([1 for char in ['长期', '.', '-'] if char in i[1][0]])>1:
                if sum([1 for char in self._char_number if char in i[1][0]])>1:
                    if sum([1 for j in i[1][0][-21:] if j in '0123456789.-'])==21:
                        ...
                        break
                    elif sum([1 for j in i[1][0][-13:] if j in '0123456789.-长期'])==13 and '长期' in i[1][0]:
